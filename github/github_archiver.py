...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--do-not-archive', dest='do_not_archive',
                        action='store_true',
                        help='if set, archival of flagged repos is not performed (off by default)')
    parser.add_argument('-d', '--delete', dest='delete',
                        action='store_true',
                        help='delete flagged repos, will ask for confirmation (off by default)')
    ...
    if "GITHUB_PASSWORD" in os.environ:
        ...
        print "GITHUB_PASSWORD environment variable is not set, so will ask for password."
    ...
    try:
        ...
        for r in repo_list:
            ...
            # Clone wiki
            # Note: looks like repo.has_wiki is always true, even if the wiki repo does not exist
            ...
            if repo.has_issues:
                if os.path.exists(os.getcwd() + "/" + r + ".issues"):
                    print "!!! Could not save " + r + "'s issues, name already exists in current directory."
                else:
                    all_issues = []
                    ...
                    print "Archived " + str(len(all_issues)) + " total issues for repo " + repo.name + "."
        ...
        if args.delete:
            ...
            for r in repo_list:
                sys.stdout.write(r + " ")
            sys.stdout.write("\nDeleting a repo on Github can't be easily undone.\n")
